...
class MainViewModel(
    ...
    fun handlePetSelected(pet: PetItemModel) =
        _screenEvent.postValue(MainScreenEvent.ShowWebSContent(pet.contentUrl))
    ...
    suspend fun loadSettings(): Boolean = withContext(Dispatchers.IO) {
        ...
        response.data?.let {
            ...
            _screenEvent.postValue(MainScreenEvent.ShowErrorScreen(it.asMessage()))
        }
